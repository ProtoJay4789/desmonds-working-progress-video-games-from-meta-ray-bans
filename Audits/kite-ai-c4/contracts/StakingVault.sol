    ...
    IStakingVaultOperations
{
    ...
    /// @inheritdoc IStakingVaultOperations
    function recoverStrandedValidatorRewards(
        bytes32
    ) external {
        _delegateToOperations();
    }
    ...
    /// @inheritdoc IStakingVaultOperations
    function recoverStrandedDelegatorRewards(
        bytes32
    ) external {
        _delegateToOperations();
    }
    ...
    function requestWithdrawal(
        ...
        if (fee > 0) {
            ...
            if (StakingVaultInternals.getAvailableStake() < fee) revert StakingVault__InsufficientBuffer();
            stakeAmount -= fee;
