...
import type { AgentTask, AgentPolicy, DeFiOpportunity, TaskAction } from './types.js';
...
export class TaskDelegator {
  ...
  private policy: AgentPolicy | null = null;
  ...
  async executeBatch(
    ...
    for (let i = 0; i < opportunities.length; i += maxConcurrent) {
      ...
      const batchResults = await Promise.all(
        batch.map(opp => this.execute(opp, options))
      );
   ...
   */
  private validateAction(action: TaskAction): { valid: boolean; reason?: string } {
    if (!this.policy) {
   ...
   */
  private async executeAction(action: TaskAction): Promise<{ txHash?: string }> {
    switch (action.command) {
      ...
      case 'bridge':
        const bridgeResult = await this.client.bridge(
          action.params.token,
          action.params.fromChain,
          action.params.amount,
