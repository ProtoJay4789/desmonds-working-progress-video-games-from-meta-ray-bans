...
executeCommand
  ...
  .action(async (from: string, to: string, amount: string, options: any) => {
...
executeCommand
  ...
  .option('--dry-run', 'Preview without executing')
  .action(async (token: string, fromChain: string, amount: string, options: any) => {
...
executeCommand
  ...
  .option('--dry-run', 'Preview without executing')
  .action(async (token: string, amount: string, to: string, options: any) => {
...
executeCommand
  ...
  .action(async (wallet: string, options: any) => {
    ...
    try {
      ...
      const results = await delegator.executeBatch(
        toExecute.map(r => r.opportunity),
        { dryRun: options.dryRun, stopOnFailure: true }
      );
