...
Exit codes:
  0 - Success (tokens are valid, refreshed if needed) OR re-auth required
  1 - Unexpected error (script crash, unexpected exception)

Note: The "re-auth required" condition (token fully revoked) is considered
an expected maintenance state and returns 0 to avoid automated incident
alerts. The JSON output will contain "needs_reauth": true to signal manual
...
def main():
    ...
    # Exit code strategy:
    #  0 - Tokens fresh OR re-auth needed (expected maintenance conditions)
    #  1 - Unexpected errors (critical, script crash, etc.)
    # This keeps the cron job "green" when a token expires, preventing
    # automated incident alerts. Manual check of logs/JSON needed to see re-auth flag.
