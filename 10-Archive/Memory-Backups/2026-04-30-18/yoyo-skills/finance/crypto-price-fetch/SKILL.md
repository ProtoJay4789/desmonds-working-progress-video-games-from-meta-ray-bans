...
- **Shell pipe security**: `curl | python3` is blocked by Hermes security scanner. Use `terminal("curl -s URL -o /tmp/file.json")` then parse separately, OR use `execute_code` with `urllib.request` directly.
- **CoinMarketCap key may be invalid/absent** — always fall back to CoinGecko
- **CoinGecko free tier** is rate-limited (~10-30 req/min) — batch requests where possible
