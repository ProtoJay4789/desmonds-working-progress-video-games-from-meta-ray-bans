...
1. **Archive yesterday's loose files**
   ...
   - Move each to `YYYY/WXX/YYYY-MM-DD/` using ISO week numbering
   - Create the folder if it doesn't exist

2. **Identify active discussions to flag**
   - Read the last 1–3 days of files in the current week folder
   - Extract: topic, owner, status, priority, and "why still active"
   - Flag anything that is CRITICAL (deadline <10 days), STALLED (>24h no movement), or awaiting a decision

3. **Prepare today's context**
   - Write `today-context.md` in today's date folder with:
     - Active discussions table
     - Flags (blockers, stale handoffs, overloaded agents)
     - Today's agenda (checklist items with owners and priorities)
     - Yesterday's archive summary (what files were moved where)

4. **Note permanent root references**
