...
- **Multiple profile config copies.** Configs exist in DMOB, Gentech, and YoYo profile directories. Vault copy ≠ any runtime copy. A cron job reads from its **owning profile's directory**, NOT the vault or another agent's dir. Always locate ALL copies first:
  ```bash
  find /root/.hermes/profiles/ -name ".lfj-aae-config.json" -o -name "lp-position-reader.py" 2>/dev/null
  ```
  Update ALL of them, not just the vault copy. Sync with `cp` after editing the vault copy.
...
- **Runtime JSON config overrides Python DEFAULT_CONFIG.** The AAE script reads `~/.hermes/scripts/.lfj-aae-config.json` at runtime. This JSON file **overrides** the Python `DEFAULT_CONFIG`. If updating position balance, token amounts, range, or shape, edit BOTH:
  - Vault Python script (`DEFAULT_CONFIG`) — `cp` to runtime
  - Runtime JSON config — `~/.hermes/scripts/.lfj-aae-config.json`
  **Debugging:** If the script ignores the updated config:
  - Check `CONFIG_FILE` path resolution (e.g., `print(CONFIG_FILE)` in the script).
  - Verify the config file exists at the expected path.
  - Look for hardcoded defaults in the script (e.g., `pos_config.get("range_low", 9.00)`).

- **`~` resolves to profile home, not `/root/`.** `~/.hermes/scripts/` resolves to `/root/.hermes/profiles/<profile>/home/.hermes/scripts/`, NOT `/root/.hermes/scripts/`. These are DIFFERENT directories. Always use:
  ```bash
  find /root/.hermes/profiles/ -name '.lfj-aae-config.json' 2>/dev/null
  ```
  Writing to `/root/.hermes/scripts/` won't affect scripts running under a profile.

- **Cron jobs read from their own profile.** The `cronjob` tool only lists jobs in your profile. Jobs in other profiles (e.g., YoYo, Gentech) are invisible. To update a job in another profile:
  ```bash
  # Manually edit the other profile's jobs.json
  cat /root/.hermes/profiles/<profile>/cron/jobs.json | python3 -c "
  import sys, json
  data = json.load(sys.stdin)
  for j in data['jobs']:
...
- **Agent memory drift.** Agent memories (e.g., `00-System/agent-profiles/gentech/memories/MEMORY.md`) may still reference the old range. Update them to prevent cross-agent confusion.
