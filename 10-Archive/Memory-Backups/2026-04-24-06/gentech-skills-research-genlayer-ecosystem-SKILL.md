...
- Flow: x402 → Apolo escrow → GenLayer adjudicates → BNB Chain settles
...
- 75% → Stake pool (validators + delegators proportional to shares)
- 10% → Validator owners (ops fee, taken before distribution)
- 10% → Developers
...
- **Slashing:** Validators + their delegators get slashed for missing execution windows or supporting fraud. 24-hour governance delay on all slashing.
...
- Install Agentic Wallet skills → authenticate with email → call any service
