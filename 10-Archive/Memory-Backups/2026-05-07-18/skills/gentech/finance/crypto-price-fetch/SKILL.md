...
When `execute_code` + `urllib.request` fails (sandbox network restrictions), use the browser as an API proxy:

```
Step 1: browser_navigate(url="https://api.coingecko.com/api/v3/simple/price?ids=avalanche-2,joe,usd-coin,bitcoin,ethereum,solana&vs_currencies=usd&include_24hr_change=true")
Step 2: browser_console(expression="document.body.innerText")
```

Returns a Python dict with all prices and 24h changes in one call. Works for CoinGecko, CoinCap, and any JSON API endpoint.
