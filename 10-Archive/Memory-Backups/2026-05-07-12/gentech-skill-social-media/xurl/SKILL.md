...
## Automation & Cron Job Health

When running xurl in cron jobs, automation scripts, or agent workflows, authentication failures can silently break data pipelines. Follow this checklist:

### Pre-flight Health Check (Run Before Any Automated xurl Command)
```bash
# 1. Verify xurl is installed
command -v xurl >/dev/null || { echo "ERROR: xurl not installed"; exit 1; }

# 2. Check auth configuration
AUTH_STATUS=$(xurl auth status 2>&1)

# 3. Parse status: look for "No apps registered" or empty/401 responses
if echo "$AUTH_STATUS" | grep -q "No apps registered"; then
    echo "CRITICAL: xurl has no authenticated apps. Run these manually:"
    echo "  xurl auth apps add <app-name> --client-id <ID> --client-secret <SECRET>"
    echo "  xurl auth oauth2 --app <app-name> [USERNAME]"
    echo "  xurl auth default <app-name>"
    exit 1
fi

# 4. Verify default app has oauth2 token (not just app credentials)
if echo "$AUTH_STATUS" | grep -q "oauth2: (none)"; then
    echo "WARNING: Default app has no OAuth2 token. Check:"
    echo "  xurl auth apps list"
    echo "  xurl auth default <app-with-token>"
    exit 1
fi

# 5. Sanity check — single cheap API call
if ! xurl whoami >/dev/null 2>&1; then
    echo "ERROR: xurl auth test failed (whoami returned non-zero)."
    echo "Re-run: xurl auth oauth2 --app <app-name>"
    exit 1
fi

echo "OK: xurl authenticated"
```

### Autonomous / Cron-Only Contexts (No User Present)
When running in a cron job or autonomous agent with no user to interact with:
- **If xurl auth fails, report the blocker and stop.** Do NOT attempt browser-based web scraping of X/Twitter as a fallback — search engines (Brave, Google, DuckDuckGo) consistently return CAPTCHAs from headless/server IPs, and X syndication endpoints are rate-limited. Wasting cycles on alternative approaches that also fail just delays the inevitable.
- The correct autonomous response is a brief status report naming the auth blocker, the fix command the admin needs to run, and an exit.
- Pre-flight check (run first, gate everything else on it):
  ```bash
  xurl whoami >/dev/null 2>&1 || { echo "BLOCKED: xurl not authenticated. Admin must run: xurl auth oauth2 --app <name>"; exit 1; }
  ```

### Common Cron Failure Patterns & Recovery
- **401 Unauthorized everywhere** → OAuth token missing, expired, or bound to wrong app profile. Re-run `xurl auth oauth2 --app <app-name>` then `xurl auth default <app-name>`.
- **Empty JSON or `{\"data\":[]}`** with no error → Auth flow succeeded but user lookup failed (X bug). Re-run with explicit username: `xurl auth oauth2 --app <app-name> YOUR_USERNAME`.
- **403 client-forbidden** → App not enrolled in X Pay-per-use plan. Move app from "Free" to "Pay-per-use" in Developer Console, then re-auth.
- **Silent empty results** → Wrong query syntax or rate-limited. Verify with `xurl whoami` first, then test query interactively before cron.

### Automation Hardening Tips
- **Always check exit code**: Wrap xurl calls: `if output=$(xurl search "..."); then ... else echo "xurl failed: $?"; fi`
- **Log auth status daily**: Cron job at 00:00: `xurl auth status >> /var/log/xurl-health.log`
- **Alert on 401s**: Gate cron execution behind the pre-flight check above. If it fails, send alert to admin — don't silently skip.
- **Never log `.xurl`**: Even in error contexts, never output `cat ~/.xurl`. Report "auth status failed" instead.

### Multi-Profile / Multi-Agent Scenarios
When multiple Hermes agents share a host, each has its own `~/.xurl` file. Never assume the system xurl is the agent's xurl. If a cron job runs as root but agents run under separate Hermes profiles, either:
- Explicitly set `XURL_CONFIG=/path/to/agent/.xurl` before running xurl, OR
- Switch to the agent's user context (`hermes run --profile <name> -- xurl ...`), OR
- Each agent runs its own xurl commands within its own profile.
