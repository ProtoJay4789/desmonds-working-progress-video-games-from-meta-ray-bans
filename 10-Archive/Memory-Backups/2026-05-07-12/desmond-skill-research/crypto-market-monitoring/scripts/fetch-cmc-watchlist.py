#!/usr/bin/env python3
"""
CMC Watchlist Fetch — reusable template
Fetches prices via CMC Pro API with CoinGecko fallback.
Returns dict: {symbol: {price, change_1h, change_24h, change_7d, volume_24h}}
"""
...
# Configuration paths
CMC_CONFIG = os.path.expanduser("~/.hermes/scripts/cmc_config.json")
STATE_FILE = os.path.expanduser("~/.hermes/scripts/.cmc-watchlist-hourly-state.json")
MOVEMENT_THRESHOLD = 1.5  # % change to trigger alert

# Watchlist — extend as needed
COINS = [
    {"symbol": "BTC",   "cmc_id": "1"},
    ...
    {"symbol": "COQ",   "cmc_id": "25967"},
    {"symbol": "ARENA", "cmc_id": "29335"},
    {"symbol": "PROPS", "cmc_id": "29618"},
    {"symbol": "LAND",  "cmc_id": "24405"},
]
...
def fetch_coingecko():
    ids = ",".join(CG_IDS[s] for s in [c["symbol"] for c in COINS] if s in CG_IDS)
...
def main():
    ...
    output = {
        "time": now_str,
        "source": source,
        "significant_movement": max_change >= MOVEMENT_THRESHOLD,
    ...
    for coin in COINS:
        ...
        if sym in prices:
            ...
            output["coins"][sym] = {
                ...
                "change_7d": f"{d['change_7d']:+.2f}%",
                "movement_since_last": f"{movements.get(sym, 0):.2f}%",
                "volume_24h": f"${d['volume_24h']:,.0f}" if d['volume_24h'] else "N/A",
            }
