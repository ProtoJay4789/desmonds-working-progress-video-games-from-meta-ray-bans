...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `Unauthorized` (401) on all commands | No OAuth token or wrong default app | Check `xurl auth status`; re-run `xurl auth oauth2 --app my-app` outside agent session |
...
### Fallback: Read tweets without xurl auth

If xurl returns 401 (not authenticated) and you need to read a tweet, use the **fxtwitter API** as a fallback:

```bash
curl -s "https://api.fxtwitter.com/SCREEN_NAME/status/POST_ID" | python3 -m json.tool
```

Key fields: `tweet.text`, `tweet.author.name`, `tweet.likes`, `tweet.media.photos[]`.

Also useful for resolving t.co short URLs:
```bash
curl -s -L -o /dev/null -w "%{url_effective}" "https://t.co/SHORT_URL"
```

**Limitations:** Cannot read X Articles (requires login), no write access, some link-only tweets return empty text. See `references/fxtwitter-fallback.md` for full details.
