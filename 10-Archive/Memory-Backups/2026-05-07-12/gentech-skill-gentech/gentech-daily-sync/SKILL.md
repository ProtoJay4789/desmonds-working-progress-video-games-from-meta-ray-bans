...
4. **Read yesterday's daily context** — Attempt to read `11-Mess Hall/YYYY/W##/YYYY-(previous-day)/today-context.md` for carry-forward items. If missing (weekend/holiday), scan backwards one day at a time until a context file is found or W##-1 week boundary reached.
...
7. **Pull active handoffs** — List files in `09-Green Room/active-handoffs/` (dated filenames); read each for cross-agent tasks. Also scan `09-Green Room/WORKFLOW-ACTIVE.md` for coordination protocols.
...
9. **Read auto-updated DeFi trackers explicitly** — 
   ...
   - `09-Green Room/D5-Strategy-Engine-Evolution.md` (milestone tracker state)
...
- **PORTFOLIO/CONTENT TRACKER SWAP**: The D5/LP trackers formerly lived in `03-Projects/DeFi/`; they now reside in `09-Green Room/` with prefixed filenames (`LP Scout — ...`, `D5-Strategy-Engine-Evolution.md`). Scan `09-Green Room/` for `lp-status-*.md` and `d5-*.md` patterns instead.
