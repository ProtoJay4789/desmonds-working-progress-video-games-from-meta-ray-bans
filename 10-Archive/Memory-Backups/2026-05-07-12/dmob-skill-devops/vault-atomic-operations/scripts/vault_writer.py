...
Provided by: vault-atomic-operations skill
Path in vault: 02-Labs/scripts/vault_writer.py
"""
...
def vault_write(
    ...
    """Atomically write to vault with audit trail. Returns status dict."""
...
---\n"""
    ...
    return {
        "status": "ok", "path": str(abs_path.relative_to(VAULT_ROOT)),
        "timestamp": timestamp, "bytes_written": len(new_content), "mode": mode,
    }


def vault_read(rel_path: str, tail: int = 0) -> Dict[str, Any]:
    """Read vault file. If tail > 0, return only last N entries."""
    abs_path = VAULT_ROOT / rel_path
    ...
    result = {
        "status": "ok", "path": rel_path, "content": content,
        "line_count": len(content.splitlines()), "entry_count": len(entries),
    }
    if tail > 0:
        result["tail_content"] = "---\n".join(entries[-tail:])
...
def vault_search(
    pattern: str, group: Optional[str] = None, limit: int = 50
) -> Dict[str, Any]:
    """Case-insensitive substring search across vault (optionally scoped to group)."""
    matches = []
        ...
        try:
            ...
            for i, line in enumerate(text.splitlines(), 1):
                if pattern.lower() in line.lower():
                    matches.append({"file": rel, "line": i, "context": line.strip()})
...
if __name__ == "__main__":
    ...
    try:
        if args.cmd == "write":
            result = vault_write(args.path, args.content, args.author, args.group, None, args.mode)
        elif args.cmd == "read":
