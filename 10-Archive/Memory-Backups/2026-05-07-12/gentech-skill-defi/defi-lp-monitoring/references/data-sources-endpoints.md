...
**Batching**: Avalanche public RPC has a payload size limit — batches >~30 items return HTTP 500. For bin scans (50-100+ calls), use sequential single calls (~0.2s each, ~20s total for ±50 bins). Batch only when <15 calls.
...
**Preferred approach**: Use `execute_code` tool with `urllib.request` — bypasses Tirith security scanner entirely and handles both HTTP fetches and JSON parsing in one call. No temp files or shell pipes needed. See main SKILL.md §Pitfalls for example pattern.
...
| Avalanche RPC | ~20/sec public | Sequential for >30 calls; batch ≤15 |
