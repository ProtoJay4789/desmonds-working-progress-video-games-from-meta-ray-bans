...
## Withdrawn Position Fields

When position is empty, replace active-position fields:

| Field | Withdrawn Value |
|-------|----------------|
| `**LP Position**` | `🚨 **EMPTY** — No active LFJ position detected on-chain` |
| `**Balances**` | replaced by `**Wallet**` only (dust amounts) |
...
| `**Recent Transactions**` | New field — list last 3-5 txns with timestamps |

D5 Milestone Alignment for withdrawn:
```
- 🚨 **No active position** — Scout tier suspended until position is re-established.
- AVAX $PRICE is {inside/outside} target band.
- IL: N/A.
- Efficiency: N/A.
```
