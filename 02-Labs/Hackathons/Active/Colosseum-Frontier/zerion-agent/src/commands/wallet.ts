...
policyCommand
  ...
  .action(async (wallet: string, options: any) => {
    ...
    try {
      ...
      if (options.chains) policy.chains = options.chains.split(',');
      if (options.maxTransfer) policy.maxTransfer = parseFloat(options.maxTransfer);
      if (options.tokens) policy.tokenAllowlist = options.tokens.split(',');
      if (options.expiry) policy.expiry = options.expiry;
