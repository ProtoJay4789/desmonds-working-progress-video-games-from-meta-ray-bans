...
import type { Position, DeFiOpportunity, WalletAnalysis } from './types.js';
...
export class OpportunityScanner {
   ...
   */
  private findIdleAssets(positions: Position[]): Position[] {
   ...
   */
  private solanaYieldOpportunities(asset: Position): DeFiOpportunity[] {
    const opportunities: DeFiOpportunity[] = [];
   ...
   */
  private evmYieldOpportunities(asset: Position): DeFiOpportunity[] {
    const opportunities: DeFiOpportunity[] = [];
    const symbol = asset.symbol?.toLowerCase();
   ...
   */
  private analyzeChainSplit(positions: Position[]): { chain: string; value: number }[] {
   ...
   */
  private bridgeOpportunities(chainSplit: { chain: string; value: number }[]): DeFiOpportunity[] {
    const primaryChain = chainSplit[0].chain;
    const opportunities: DeFiOpportunity[] = [];

    for (let i = 1; i < chainSplit.length; i++) {
      const secondary = chainSplit[i];
      if (secondary.value < MIN_VALUE_USD) continue;
   ...
   */
  private rankOpportunities(opportunities: DeFiOpportunity[]): Recommendation[] {
    ...
    for (const opp of opportunities) {
      ...
      if ((riskLevels[opp.risk] ?? 0) > maxRisk) continue;

      let priority: 'high' | 'medium' | 'low' = 'low';
      if (netBenefit > 0.05) priority = 'high';
      else if (netBenefit > 0.02) priority = 'medium';
   ...
   */
  private generateReason(opp: DeFiOpportunity): string {
    switch (opp.type) {
      case 'yield':
