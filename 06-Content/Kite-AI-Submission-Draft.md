...
**Track:** Novel Track — Kite AI Global Hackathon  
**Deadline:** May 11, 2025  
**Submitted by:** Gentech Labs  
...
AI agents are about to autonomously execute billions of dollars in transactions. But there's no trust layer for agent payments.
...
- **Who pays? How do they know the agent actually did the work? What if the agent hallucinates the result?**

Today, agents either:
- Run on prepaid budgets (no proof of work)
- Require human approval per transaction (not autonomous)
- Use traditional payment rails (slow, KYC-heavy, not agent-native)
...
We built AgentEscrow to be **chain-agnostic** from day one. The core contracts are pure Solidity + OpenZeppelin — deployable to any EVM chain. Chain-specific integrations (gasless transfers, AA SDK, attestations) are swappable modules.
...
| `TECHPaymentRouter.sol` | x402 payment middleware | ~180 | 14/14 ✅ |
...
- **Daily spending limits** — Agent-level rate limiting prevents runaway costs

---

## 🔗 Kite AI Integration

This is where it gets interesting. We designed AgentEscrow to be **modular** — swap in Kite's unique primitives:
...
### What We Built (14/14 Tests Passing)

We have a fully working AgentEscrow system deployed on Avalanche Fuji testnet. The code is production-grade.
...
**Kite AI is our next target.** The architecture is designed for this — swap Avalanche RPC for Kite testnet, add Agent Passport identity, and post attestations. We'd estimate **2-3 days of focused work** to port.
...
- **Architecture:** `AgentEscrow-Project.md` in Gentech Vault
